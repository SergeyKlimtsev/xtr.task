...
class Vacancies extends React.Component {
    constructor(props) {
        super(props);
        props.refreshVacancies();
    }

    render() {
        const {vacancies, pageSize, page, pageCount, refreshVacancies, changePageSize, changePage} = this.props;
                ...
                <Table hover>
                    ...
                    <tbody>{
                        vacancies.map((item, i)=>(<tr key={'row_'+i}>
                            <td>{item.name}</td>
                            <td>{(item.salaryFrom ? `От ${item.salaryFrom}` : '') + (item.salaryTo ? ` до ${item.salaryTo}` : '')}</td>
